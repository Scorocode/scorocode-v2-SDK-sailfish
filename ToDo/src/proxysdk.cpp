...
{
    //Set you appId
    sdk = ScorocodeSDK::initApp("250d3a8443ed4e8f8d4cfab512fcb35a", m_sslConfig, ".t8.scorocode.ru");
    if (sdk)
    {
        init();
    }
...
void ProxySDK::singin(const QString &user, const QString &password)
{
...
void ProxySDK::singup(const QString &user, const QString &password)
{
...
{
    qDebug() << "get record: db type" << type << "name" << db_name << "schema name" << schema << "table name" << table << "additional" << additional;
    if (!m_token.isNull())
    {
...
void ProxySDK::singinDone(int errorCode, QJsonDocument data)
{
    ...
    if (0 == errorCode)
    {
        ...
        m_user = object.value("user").toObject().value("email").toString();
        ...
        emit singinSuccefull(true);
    }
    emit singinSuccefull(false);
}

void ProxySDK::singupDone(int errorCode, QJsonDocument data)
{
    if (0 == errorCode)
    {
        QJsonObject object = data.object();
        QString userInfo = object.value("user").toObject().value("email").toString();
        QJsonObject recordObj;
        recordObj.insert("userName", object.value("user").toObject().value("email"));
        recordObj.insert("email", object.value("user").toObject().value("email"));
        QJsonDocument record(recordObj);
        database->insertRecord(databaseType, databaseName, databaseSchema, "users", record.toJson(QJsonDocument::Compact));
        emit singupSuccefull(true, userInfo);
    }
    else
    {
        qDebug() << "singupSuccefull false";
        emit singupSuccefull(false, "Error");
    }
    singinDone(errorCode, data);
}
...
void ProxySDK::init()
{
    ...
    connect(sdk, &ScorocodeSDK::tokenExpaired, this, &ProxySDK::tokenExpired);

    connect(auth, &Auth::signinDone, this, &ProxySDK::singinDone);
    connect(auth, &Auth::signupDone, this, &ProxySDK::singupDone);
    connect(auth, &Auth::networkError, this, &ProxySDK::authError);
    ...
    connect(database, &Database::getRecordDone, this, [this](int errorCode, QJsonDocument data, QString tableName){
        qDebug().noquote() << "error code" << errorCode << data.toJson();
        emit getRecordListDone(data.toJson(), tableName);
    });
    connect(database, &Database::updateRecordDone, this, [this](int errorCode, QJsonDocument data, QString tableName){
        qDebug().noquote() << "error code" << errorCode << data.toJson();
        emit updateRecordDone(data.toJson(), tableName);
    });
    connect(database, &Database::insertRecordDone, this, [this](int errorCode, QJsonDocument data, QString tableName){
        qDebug().noquote() << "error code" << errorCode << data.toJson();
        emit insertRecordDone(data.toJson(), tableName);
    });
